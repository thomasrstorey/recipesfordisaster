...
def addCastMod(scn, obj):
    bpy.ops.object.select_all(action='DESELECT')
    obj.select = True
    scn.objects.active = obj
    bpy.ops.object.modifier_add(type='CAST')
    obj.modifiers["Cast"].cast_type = 'CYLINDER'
    obj.modifiers["Cast"].factor = 0.50 + (random.random() * 0.70)
    obj.select = False
...
def main():
    ...
    print("wrapping " + ", ".join(inputs))
    execute(inputs, output)
    print("wrapped " + ", ".join(inputs))
