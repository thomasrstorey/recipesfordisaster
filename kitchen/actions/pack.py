...
def getObject(objdir, objname):
    if (bpy.data.objects.get(objname) == None):
        ...
        obj = bpy.data.objects[objname].copy()
        obj.data = bpy.data.objects[objname].data.copy()
...
def execute(inputs, output):
    ...
    for objname in inputs:
        ...
        objs = []
        for i in range(0, 3):
            objs.append(getObject(objdir, objname))
            objs[i].location = Vector([0,0,0+(0.33*i)])
