...
def addTiltMod(scn, obj):
    bpy.ops.object.select_all(action='DESELECT')
    obj.select = True
    scn.objects.active = obj
    bpy.ops.object.modifier_add(type='SIMPLE_DEFORM')
    obj.modifiers["SimpleDeform"].deform_method = 'BEND'
    obj.modifiers["SimpleDeform"].angle = 3.14159
    obj.modifiers["SimpleDeform"].limits = (0.66, 1.00)
    obj.select = False
...
def main():
    ...
    print("tilting " + ", ".join(inputs))
    execute(inputs, output)
    print("tilted " + ", ".join(inputs))
