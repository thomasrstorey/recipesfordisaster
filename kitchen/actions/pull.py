...
def scaleObjectRandomZ(obj):

    # decompose world_matrix's components, and from them assemble 4x4 matrices
    orig_loc, orig_rot, orig_scale = obj.matrix_world.decompose()

    # define the scale
    xscale = Matrix.Scale(random.random()*2.0,4,(1,0,0))
    yscale = Matrix.Scale(random.random()*2.0,4,(0,1,0))
    zscale = Matrix.Scale(random.random()*2.0,4,(0,0,1))
    scale_mat =  xscale * yscale * zscale
    ...
    orig_scale_mat = xscale * yscale * zscale
    # assemble the new matrix
    obj.matrix_world = orig_loc_mat * orig_rot_mat * scale_mat * orig_scale_mat
...
def main():
    ...
    print("pulling " + ", ".join(inputs))
    execute(inputs, output)
    print("pulled " + ", ".join(inputs))
